...
`.env`ファイルを編集し、運営から配布されたAPIキーを設定してください。
...
生成完了後、`presentation/index.html` をブラウザで開いて確認
...
1. **名前**
2. **サービス名**
3. **PR/FAQ** - `ideation/step3_prfaq.md` の内容
4. **ビジネスモデルキャンバス**（オプション）
5. **モックのスクリーンショットまたは動画**
6. **スライド** - `presentation/index.html` のスクリーンショット
7. **実現したいこと（想い）** - 100文字程度
8. **チームメンバー募集** - Yes / No
...
1. **課題と顧客**（30秒） - 誰のどんな課題を解決するか
2. **解決策の概要**（1分） - サービスの主要機能と独自の強み
3. **モックデモ**（1分） - 画面共有でモックを実際に操作
4. **募集の呼びかけ**（30秒） - チームメンバーを募集する場合
